...
from ..classes.preference_preorder import \
    PreferencePreorder
from ..classes.strict_rule import StrictRule
...
class LayeredArgumentationSystemGenerator:
    ...
    def __init__(self, nr_of_literals: int, nr_of_rules: int,
        ...
        """
        Construct a generator for making random Argumentation Systems with a
        layered structure

        :param nr_of_literals: The desired number of literals.
        :param nr_of_rules: The desired number of rules.
        :param rule_antecedent_distribution: Number of rules (value) with
        specific number of antecedents (key)
        :param literal_layer_distribution: Number of literals (value) in a
        specific layer (key)
        ...
        required_rules = sum([
            value for key, value in literal_layer_distribution.items()
            if key != 0])
        if nr_of_rules < required_rules:
            raise ValueError('You need more rules to enable this literal '
                             'layer distribution.')
        ...
        if any([x + 1 != y for x, y in zip(possible_layers[:-1],
                                           possible_layers[1:])]):
            raise ValueError('Each layer between the minimum and maximum '
                             'should have a value.')
        if sum(literal_layer_distribution.values()) != nr_of_literals:
            raise ValueError('The sum of the literal layer distribution '
                             'should equal the language size.')
        self.literal_layer_distribution = literal_layer_distribution

        if sum(rule_antecedent_distribution.values()) != nr_of_rules:
            raise ValueError('The sum of the rule antecedent distribution '
                             'should equal the rule size.')
        ...
        if any([nr_antecedents > nr_of_literals / 2
            ...
            raise ValueError('Rules cannot have more antecedents than half'
                             ' of the language size.')

        if strict_rule_ratio < 0 or strict_rule_ratio > 1:
            raise ValueError('Strict rule ratio should be between zero and '
                             'one.')
        self.strict_rule_ratio = strict_rule_ratio

    def generate(self, return_layered_language: bool = False,
                 add_rule_preferences=True):
        max_remaining_tries = 25
        while max_remaining_tries > 0:
            try:
                max_remaining_tries -= 1
                layered_language, contraries = \
                    self._generate_language_and_contradictories_initial()
                strict_rules, defeasible_rules, def_rule_preferences = \
                    self._generate_rules(layered_language,
                                         add_rule_preferences, contraries)
                language = {str(literal): literal
                            for literals in layered_language.values()
                            for literal in literals}

                argumentation_system = ArgumentationSystem(
                    language, contraries, strict_rules, defeasible_rules,
                ...
                pass
        raise ValueError('Could not generate an ArgumentationSystem with '
                         'these properties.')
    ...
    def _generate_language_and_contradictories_initial(self) -> \
        ...
        potential_queryable_size = int(positive_language_size / 2)
        for pos_literal_index in range(positive_language_size):
            ...
            if pos_literal_index < potential_queryable_size:
                # To make sure that there are enough literals that are on
                # level 0 AND their contradictories are on level 0, so they
                # can become queryables.
                new_literal_positive_layer = new_literal_negative_layer = 0
                layers[0] -= 2
            else:
                new_literal_positive_layer = random.choices(
                    list(layers.keys()), list(layers.values()), k=1)[0]
                layers[new_literal_positive_layer] -= 1
                new_literal_negative_layer = random.choices(
                    list(layers.keys()), list(layers.values()), k=1)[0]
            ...
            layered_language[new_literal_positive_layer].append(
                new_literal_positive)
            layered_language[new_literal_negative_layer].append(
                new_literal_negative)

        return layered_language, contradictories

    def _generate_rules(self, layered_language: Dict[int, List[Literal]],
                        add_rule_preferences: bool,
        ...
        necessary_consequents = [
            (literal, layer)
            for layer, literals in layered_language.items()
            for literal in literals
            if layer > 0]
        for consequent, consequent_layer in necessary_consequents:
            ...
            if not layered_language[consequent_layer - 1]:
                raise ValueError(
                    'Could not add a rule with the required number of '
                    'literals')
            highest_antecedent = random.choice(
                layered_language[consequent_layer - 1])
            antecedents.append(highest_antecedent)

            nr_of_antecedents = random.choices(
                list(r_a_d.keys()), list(r_a_d.values()), k=1)[0]
                ...
                for other_layer, other_literals in layered_language.items():
                    if other_layer < consequent_layer:
                        for other_literal in other_literals:
                            if other_literal not in antecedents and \
                                    other_literal != consequent and \
                                    all([contrary not in antecedents and
                                         contrary != consequent
                                         for contrary in contradictories[
                                             str(other_literal)]]):
                                antecedent_candidates.append(other_literal)

                if not antecedent_candidates:
                    raise ValueError('Could not add a rule with the required '
                                     'number of literals')
            ...
            rule_will_be_strict = random.choices(
                [True, False],
                weights=[
                    self.strict_rule_ratio, 1 - self.strict_rule_ratio])[0]
            if rule_will_be_strict:
                new_rule = StrictRule('r' + str(len(strict_rules)),
                                      set(antecedents), consequent)
                strict_rules.append(new_rule)
            else:
                new_rule = DefeasibleRule('d' + str(len(defeasible_rules)),
                                          set(antecedents), consequent)
        ...
        while len(strict_rules) + len(defeasible_rules) < self.nr_of_rules:
            if not necessary_consequents:
                raise ValueError('It is not possible to generate the required '
                                 'number of rules.')
            consequent, consequent_layer = random.choice(necessary_consequents)

            # Find antecedents
            nr_of_antecedents = random.choices(list(r_a_d.keys()),
                                               list(r_a_d.values()), k=1)[0]
            r_a_d[nr_of_antecedents] -= 1
        ...
        if add_rule_preferences:
            ...
            contradicting_rules = {
                (rule_a, rule_b)
                for rule_a, rule_b in itertools.combinations(
                    shuffled_defeasible_rules, 2)
                if rule_a.consequent in contradictories[
                    str(rule_b.consequent)]}
            leq_rules = random.sample(list(contradicting_rules),
                                      int(len(contradicting_rules) / 2))
            for geq_tuple in leq_rules:
                rule_preferences.append(geq_tuple)
            rule_preferences.fix_transitivity()
